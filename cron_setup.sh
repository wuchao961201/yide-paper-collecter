...
# 获取脚本的绝对路径
SCRIPT_PATH=$(realpath demo_PaperCollector_Yide.py)
WORKSPACE_PATH=$(dirname "$SCRIPT_PATH")

# 创建临时crontab文件
echo "# 每天上午10点执行论文收集器脚本" > /tmp/paper_collector_cron
echo "0 10 * * * cd $WORKSPACE_PATH && /usr/bin/python3 $SCRIPT_PATH >> $WORKSPACE_PATH/cron_execution.log 2>&1" >> /tmp/paper_collector_cron
